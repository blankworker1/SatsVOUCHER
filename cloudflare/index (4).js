...
<div id="loginScreen" class="login-wrap">
  <div class="login-card">
    ...
    <div class="pkb-grid">
      <button class="pkb" data-k="1">1</button><button class="pkb" data-k="2">2</button><button class="pkb" data-k="3">3</button>
      <button class="pkb" data-k="4">4</button><button class="pkb" data-k="5">5</button><button class="pkb" data-k="6">6</button>
      <button class="pkb" data-k="7">7</button><button class="pkb" data-k="8">8</button><button class="pkb" data-k="9">9</button>
      <button class="pkb" data-k="0" style="grid-column:2">0</button>
      <button class="pkb pkb-del" data-k="del">&#9003;</button>
    </div>
...
// ── PIN entry
var pin = '';
document.querySelector('.pkb-grid').addEventListener('click', function(e) {
...
function renderDashboard(treasury, vouchers, settings, coins) {
  ...
  if (!coins.length) {
    ...
    html += '<span style="font-family:'DM Mono',monospace;font-size:13px;color:var(--gold)">'+sats(scActiveSats)+'</span></div>';
    ...
    html += '<span style="font-family:'DM Mono',monospace;font-size:13px;color:#555">'+sats(scRedeemedSats)+'</span></div>';
    html += '</div>';
  }
  html += '</div>';
...
export default {
    async fetch(request, env) {
        ...
        if (path.startsWith("/c/") && request.method === "GET") {
            const uid = decodeURIComponent(path.slice(3));
        ...
        if (path.startsWith("/satscash/") && request.method === "GET" && path !== "/satscash/list") {
            try {
                const uid = decodeURIComponent(path.slice(10));
                const raw = await env.VOUCHERS.get(`satscash:${uid}`);
        ...
        if (path.startsWith("/lnurlsc/") && !path.includes("callback") && request.method === "GET") {
            try {
                const uid = decodeURIComponent(path.slice(9));
                const raw = await env.VOUCHERS.get(`satscash:${uid}`);
        ...
        if (path.startsWith("/lnurlsc/callback/") && request.method === "GET") {
            try {
                const uid = decodeURIComponent(path.slice(18));
                const pr = url.searchParams.get("pr");
                const raw = await env.VOUCHERS.get(`satscash:${uid}`);
                if (!raw) return new Response(JSON.stringify({status:"ERROR",reason:"Not Found"}),{headers:jh});
                const coin = JSON.parse(raw);
