...
async function loadDashboard() {
  ...
  try {
    ...
    renderDashboard(treasury, vouchers, settings);
  } catch(e) {
...
function renderDashboard(treasury, vouchers, settings) {
  ...
  var liabilitySats = Math.round((agg.active.btc + agg.pending.btc) * 100000000);
...
export default {
    async fetch(request, env) {
        ...
        // ── ONE-TIME PURGE — DELETE /admin/purge-vouchers
        // Remove this route after use
        if (path === "/admin/purge-vouchers" && request.method === "DELETE") {
            try {
                let deleted = 0;
                let cursor = undefined;
                do {
                    const list = await env.VOUCHERS.list({ prefix: "voucher:", limit: 1000, cursor });
                ...
                return new Response(JSON.stringify({ status: "OK", deleted }), { headers: jh });
            } catch(e) {
                return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: jh });
            }
        }
